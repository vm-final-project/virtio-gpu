 ...
 *
 * Opens the native Venus Vulkan driver (libukvulkan_venus) over libukvirtio_gpu,
 * enumerates the vkmark scene list, and prints `pass-substrate`. Scene fps
 * requires a real Venus render payload and is intentionally not claimed here.
 * Upstream vkmark: https://github.com/vkmark/vkmark (LGPL-2.1+).
 */
#include <stdint.h>
#include <stdio.h>
...
int main(void)
{
    ...
    int rc = uk_vulkan_venus_open(&venus, 0);
    if (rc) {
        printf("uk-vkmark: g6=blocked:venus-open-failed rc=%d\n", rc);
        VKMARK_BLOCK("Venus driver open failed; requires virtio-gpu-gl-pci");
    }

    struct uk_vulkan_venus_info info;
    uk_vulkan_venus_get_info(&venus, &info);
    ...
    printf("uk-vkmark: ctx_initialized=%d rendering_status=%s\n",
           venus.ctx_ready, info.rendering_status);
