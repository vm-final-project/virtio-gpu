# app-vulkan-sample — native Venus Vulkan smoke test substrate port
...
- `Config.uk` — declares `CONFIG_APP_VULKAN_SAMPLE` and selects `libukvirtio_gpu` (and probes the Venus capset directly)
- `Makefile.uk` — registers with `addlib`, uses `APPVULKAN_SAMPLE_*` variables, lists `main.c`; Vulkan loader not vendored
