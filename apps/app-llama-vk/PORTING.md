# app-llama-vk — llama.cpp Vulkan / Venus / SPIR-V port
...
(`results/llama/llama_server_vk.json`; `make llm-server-vk-check` →
...
- Repository: <https://github.com/ggml-org/llama.cpp> (same as `app-llama-cpu`).
...
|---|---|---|---|
| Bench | `kraft/Kraftfile.llama-vk` | `CONFIG_APP_LLAMA_VK_MODE_BENCH=y` | `llm.bench.vk` |
| Server | `kraft/Kraftfile.llama-vk-server` | `CONFIG_APP_LLAMA_VK_MODE_SERVER=y` | `llm.server.vk` |
...
```sh
make llama-vk-build               # bench image (default)
make llama-vk-server-build        # server-mode image
make llama-vk-run                 # may stay blocked on hosts without an EGL render node
make llama-vk-check
```

Kraftfiles: `kraft/Kraftfile.llama-vk{,-server}`. Single entrypoint; no shell.
...
- `results/llama/llama_vk.json` carries the canonical row.
...
and the matching `results/llama/llama_vk.json`,
`results/llama/llama_server_vk.json`, and
`results/llama/env10_real.json`.

## Porting boundary

- Vulkan/Venus appliance only; CPU work belongs to `app-llama-cpu`.
...
- `Config.uk` declares `CONFIG_APP_LLAMA_VK` and selects the
