...
|---|---|---|
| `apps/app-llama-cpu` | Unikraft app | CPU bench-only and server-only single-application images. |
| `apps/app-llama-vk` | Unikraft app | Vulkan bench-only and server-only single-application images using upstream ggml-vulkan. |
...
2. Link through `apps/app-llama-cpu/Makefile.uk`.
3. Select exactly one mode in Kconfig/Kraftfile:
   - `CONFIG_APP_LLAMA_CPU_MODE_BENCH=y` for bench-only.
   - `CONFIG_APP_LLAMA_CPU_MODE_SERVER=y` for server-only.
...
A passing runtime must emit `evidence_id=llama-cpu`. Without that line,
