...
from ..core.artifacts import make_artifact, utc_now, write_json
...
def makefile_targets() -> set[str]:
    text = "\n".join(
        read(path) for path in [ROOT / "Makefile", *sorted((ROOT / "mk").glob("*.mk"))]
    )
    targets: set[str] = set()
...
def main() -> int:
    ...
    scripts = {
        ...
        for p in (ROOT / "scripts" / "vogue").rglob("*.py")
        if "__pycache__" not in p.parts
    }
    required_scripts = {
        "scripts/vogue/__main__.py",
        "scripts/vogue/cli.py",
        "scripts/vogue/core/artifacts.py",
        "scripts/vogue/commands/eval_matrix.py",
        "scripts/vogue/commands/venus_probe.py",
        "scripts/vogue/commands/venus_perf.py",
        "scripts/vogue/commands/vulkan_perf.py",
        "scripts/vogue/commands/server_vk_check.py",
        "scripts/vogue/commands/server_vk_throughput.py",
        "scripts/vogue/commands/current_stage.py",
    }
    ...
    payload = make_artifact(
        source="python3 -m scripts.vogue report stage",
