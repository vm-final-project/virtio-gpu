...
from ..core.artifacts import load_json, rows_by_status, utc_now, write_json
...
def llama_vulkan_rows() -> list[Row]:
    ...
    return [
        Row("host.baseline.vk",
            ...
            "Run python3 -m scripts.vogue capture host-vk once on the evaluation host."),
        Row("host.vk.probe",
            ...
            "and run the canonical Vulkan probe."),
        Row("bld.host.vk",
            ...
            "compute subset and run make llama-vk-build."),
...
def upstream_llama_rows() -> list[Row]:
    ...
    return [
        Row("llm.bench.cpu",
            ...
            "Run make llama-cpu-build; upstream sources compile in-tree."),
        Row("llm.server.cpu",
...
def write_outputs(rows: list[Row], out_dir: Path | None) -> dict:
    ...
    payload = {
        "metadata": {
            ...
            "source": "python3 -m scripts.vogue evaluate matrix",
            "version": 1,
        },
        "summary": {
