...
Model delivery matches apps/app-llama-cpu/common.h: a 9pfs share tagged
...
Outcomes (honest, never faked):
  pass                       guest printed `uk-llama-cpu: pp512=.. tg128=..` + PASS
...
def _write(payload: dict) -> None:
    ...
    (RESULTS / "llama_cpu.json").write_text(json.dumps(payload, indent=2) + "\n")


def _blocker(status: str, next_step: str) -> int:
    _write({
        "schema": "llama/upstream-runtime.v2",
        "evidence_id": "llama-cpu",
        "status": status,
        "pass": False,
        "generated_utc": _now(),
...
def main() -> int:
    ...
    if not qemu:
        return _blocker("blocked:qemu-missing", "Install qemu-system-x86 and rerun make llama-cpu-run.")
    if not IMAGE.exists():
    ...
    (RESULTS / "llama_cpu.log").write_text(out)
    (RESULTS / "llama_cpu_serial.log").write_text(out)
    m = re.search(r"uk-llama-cpu: pp512=([0-9.]+) tg128=([0-9.]+)", out)
    passed = "uk-llama-cpu: PASS evidence_id=llama-cpu" in out
    if not (m and passed):
        return _blocker("blocked:no-pass-line",
                        "Inspect results/llama/llama_cpu.log; the guest did not emit a PASS marker.")
    ...
    _write({
        ...
        "model": model.name,
        "run_log": "results/llama/llama_cpu.log",
