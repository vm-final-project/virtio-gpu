...
| Paper generated-table source paths | Partially resolved: `paper/generated/app-multi-env-table.typ` and `paper/generated/multi-env-bench-table.typ` now regenerate from current matrix data, but PDF rebuild is still blocked by the local `typst` snap environment. | Keep generator inputs on current matrix sources. Unblock the PDF build by using a non-snap `typst` binary on `PATH` (local tarball/package-manager install, or checked-in tool wrapper) and make `make paper` prefer `TYPST=/path/to/typst` when provided. | `make paper-check`; then `TYPST=/path/to/typst make paper` after replacing the broken snap runtime. |
