...
| `unikraft_alignment` | `pass` | results/stage/unikraft_alignment_latest.json | Unikraft design-rule alignment gate passes |
| `stage_audit` | `pass` | results/stage/stage_audit_latest.json | Current-stage audit passes |
| `benchmark_summary` | `pass` | rows=8 results/benchmarks/benchmark_summary_latest.json | Benchmark summary exists with native app and Venus readiness rows |
| `evaluation_matrix` | `pass` | rows=27 missing=[] | Evidence matrix contains every supported pass/blocked claim row |
| `pass_rows` | `pass` | results/vogue_latest_evaluation_matrix.json | Core supported rows pass; upstream llama.cpp rows accept structured blockers for missing QEMU/Venus images |
...
| `venus_blocker_recorded` | `pass` | results/venus/qemu_2d_probe_latest.json; results/vogue_latest_evaluation_matrix.json | QEMU Venus probe artifact recorded with a structured status |
| `real_path_selected` | `pass` | results/venus/real_path_check_latest.json | Production Kraft/config/build artifacts use the real VirtIO-GPU backend |
