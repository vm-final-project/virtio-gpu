...
*Update (real Venus bring-up).* On the evaluation host with the locally built QEMU 11.0.1 (`virtio-gpu-gl-pci,venus=true`), the Venus-enabled virglrenderer 1.11, and four NVIDIA Tesla V100 GPUs, the previously documented modern PCI blocker is resolved: Unikraft's `libvirtio_pci` now accepts the modern VirtIO device ID `0x1050`, so the appliances boot directly into the real `virtio-gpu-gl` Venus path instead of returning the earlier `BLOCKED` modern-PCI status. With this, the rows that previously read `blocked:probe-incomplete` / `blocked:wrong-domain-artifact` / `blocked:venus-compute-dispatch-incomplete` now PASS against same-run evidence: `xport.qemu-vgpu` and `proto.venus-ring` enumerate the real device and pass a QMP colour-band frame proof (`gfx.kmscube.frame` captures a host GL scanout read-back); `host.vk.probe` reads the real `Tesla V100-SXM2-16GB` device name back over a Venus reply round-trip; and the upstream llama.cpp Vulkan bench appliance (`llm.bench.vk`, `llm.bench.vk.real`) runs end-to-end on the V100 over Venus (`pp512=247.4 t/s`, `tg128=3.4 t/s`), with the server appliance (`llm.server.vk`) loading all 28 layers onto the GPU and reaching model-loaded readiness. The full evidence matrix is 27/27 PASS; see `docs/VENUS-BRINGUP.md` for the host stack and the guest-side fixes (thread-stack sizing for ggml-vulkan's concurrent pipeline compilation, the host virglrenderer reinstall, and the egl-headless GL screendump read-back).
...
- `llm.bench.vk` and `llm.bench.vk.real` are PASS on the evaluation host with real Venus artifacts. The performance issue is that prefill is strong (`pp512=247.4`) while token generation remains slower than CPU (`tg128=3.4` vs `7.7`), so future work is optimization, not unblock.
...
The current 27-row matrix (27 PASS, 0 blocked on the evaluation host) establishes the runtime path; the next work is to keep regenerated summaries aligned, add the missing ENV9 llvmpipe artifact path, preserve the current-stage real-path hygiene gate, and optimize the slower token-generation path.
