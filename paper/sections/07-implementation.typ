...
The current code should be read as five layers of evidence. First, the 2D/software-render layer is executable under native tests and application harnesses. Second, the 3D/Venus transport layer is implemented as real control-queue code and checked by ABI/static/readiness scripts. Third, QEMU/Venus transport is still evidence-gated: the modern VirtIO-PCI work is substrate evidence, but the generated matrix records `xport.qemu-vgpu` as `blocked:probe-incomplete` until a complete row-compatible probe artifact exists. Fourth, the Mesa-compatible Venus ring-buffer protocol is implemented in `libukvenus` and passes native protocol checks; same-run QEMU transport/display artifacts are not promoted unless the current appliance run also carries matching runtime evidence. Fifth, the virgl Gallium command stream encoder (`virgl_encoder.c`) is implemented in `libukvirtio_gpu` and passes native checks: it formats CREATE_OBJECT/SURFACE, SET_FRAMEBUFFER_STATE, and CLEAR command streams per the virgl protocol. Current K1 QEMU submit/frame claims are governed by the generated evidence matrix; in this revision those appliance rows are `blocked:missing-pass-marker` and must be rerun before being treated as PASS.
