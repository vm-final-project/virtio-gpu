...
#figure(
  table(
    ...
    [GGML-VirtGPU], [VirtIO-GPU ML compute offload], [Target extension; `libukvirtio_gpu` (+ upstream `uksglist`) replaces Linux DRM dependency; vk.ggml-dispatch static-ICD dispatch layer (`libukggml_vulkan`, 80+ stubs) builds and passes native tests],
...
The central dependency for K1 accelerated rendering is now satisfied on the evaluation host: xport.qemu-vgpu, Venus bootstrap command serialization, the Mesa ring-buffer protocol, and the KMSCube submit/frame rows all have current same-run evidence. The architecture VOGUE has built for the 2D path (scatter-gather backing via upstream `uksglist`, protocol command staging, fence synchronization in `libukvirtio_gpu`) is the same architecture required for 3D; future work is broader app coverage and performance rather than first transport proof.
