...
*Venus.*
Venus @venus provides a VirtIO-GPU Vulkan serialization protocol. Guest Vulkan API calls are serialized by the Mesa guest driver and forwarded through VirtIO-GPU command streams; virglrenderer deserializes and dispatches them as host Vulkan calls. Venus requires blob resources and host-visible memory for zero-copy command and buffer sharing. VOGUE now targets the Venus transport substrate — blob resources, context operations, UUIDs, map/unmap, and `SUBMIT_3D` — with verified wire-format correctness: the encoder uses PACKED layout (no inter-field alignment padding) and encodes array presence as uint32 `array_size` values per Mesa's `vn_encode_array_size()` protocol, matching virglrenderer's decoder (`vkr_context_submit_cmd`). VOGUE does not yet provide a complete Mesa/Venus guest driver or a running Vulkan application. Venus is the correct path for full GPU-accelerated rendering; the current generated matrix keeps `xport.qemu-vgpu` as `blocked:probe-incomplete`, wire-format encoding is correct, the Mesa-compatible ring protocol passes native tests, and the remaining gap is wiring that path into the guest appliance with same-run transport, frame, and host-processing proof.
...
#figure(
  table(
    ...
    [QEMU `virtio-gpu-gl` / Venus], [Host 3D/Vulkan acceleration backend], [Current matrix: `blocked:probe-incomplete` until a complete QEMU GL/Venus probe exists. Host GPU requirement: any Vulkan 1.1 driver (ANV/RADV/NVIDIA/Lavapipe) on Linux with `VK_KHR_external_memory_fd`. Use a GL-capable QEMU display backend for virtio-gpu-gl/Venus workloads.],
...
The central dependency for K1 accelerated rendering is now a current, same-run QEMU GL/Venus artifact: the generated matrix keeps xport.qemu-vgpu as `blocked:probe-incomplete`, Venus bootstrap command serialization has verified native wire-format correctness (proto.venus-enc = PASS; PACKED encoding and uint32 `array_size` fields), and the Mesa ring-buffer protocol is implemented in `libukvenus` (proto.venus-ring = PASS for native checks). The Unikraft appliance must still be rebuilt and rerun so the serial log, submit row, and pixel proof share the same run identity before frame or transport claims are promoted. The architecture VOGUE has built for the 2D path (DMA buffer management in `libukdma`, protocol command staging, fence synchronization in `libukvirtio_gpu`) is the same architecture required for 3D, but advancing beyond the current evidence boundary requires fresh runtime artifacts rather than paper-only promotion.
