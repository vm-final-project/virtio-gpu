/*
 * Mesa alignment:
 *   Analogue: src/virtio/vulkan/vn_ring.c and vn_renderer_virtgpu.c.
 *   Same: Venus capset query, context bootstrap, host-visible ring layout,
 *         head/tail/status model, and command wait model.
 *   VOGUE adaptation: native Unikraft blob mapping through libukvirtio_gpu.
 */
/*
...
found:
	...
	if (caps)
		uk_venus_decode_capset(caps, raw, actual);
	return 0;
}
			   ...
			   struct uk_venus_ring *ring)
{
	...
	if (ring->ctx.created && ring->blob.created)
		(void)uk_virtio_gpu_gl_context_detach_resource(dev, &ring->ctx,
							ring->blob.resource_id);
	if (ring->ctx.created && ring->reply_blob.created)
		(void)uk_virtio_gpu_gl_context_detach_resource(dev, &ring->ctx,
							ring->reply_blob.resource_id);
	if (ring->reply_blob.mapped)
		(void)uk_virtio_gpu_gl_blob_unmap(dev, &ring->reply_blob);
	if (ring->reply_blob.created)
		(void)uk_virtio_gpu_gl_blob_destroy(dev, &ring->reply_blob);
	if (ring->blob.mapped)
		(void)uk_virtio_gpu_gl_blob_unmap(dev, &ring->blob);
				   ...
				   struct uk_venus_ring *ring)
{
	...
	memset(&ring->reply_blob, 0, sizeof(ring->reply_blob));
	rc = uk_virtio_gpu_gl_blob_create_with_ctx(dev, ring->ctx.id,
	...
	rc = uk_virtio_gpu_gl_blob_map(dev, &ring->reply_blob);
	if (rc)
		goto fail_blob;
	if (!ring->reply_blob.mapped_addr ||
	    ring->reply_blob.mapped_size < UK_VENUS_REPLY_BLOB_SIZE) {
		rc = -ENOTSUP;
		goto fail_unmap;
	}
	...
	if (rc)
		goto fail_detach;
	uk_venus_encode_vkSetReplyCommandStreamMESA(&enc,
						    ring->reply_blob.resource_id,
						    0,
						    ring->reply_blob.mapped_size);
	if (enc.overflow) {
		rc = -ENOSPC;
		goto fail_detach;
	}

	memset(ring->reply_blob.mapped_addr, 0,
	       (size_t)ring->reply_blob.mapped_size);
	rc = uk_virtio_gpu_gl_context_submit(dev, &ring->ctx,
					     enc.buf, enc.pos, &fence);
	if (rc)
		goto fail_detach;

	ring->reply_base = (uint8_t *)ring->reply_blob.mapped_addr;
	ring->reply_size = (size_t)ring->reply_blob.mapped_size;
	ring->reply_pos = 0;
	ring->reply_ready = 1;
	return 0;
 ...
 */
/* A single persistent ring-owned reply stream, created on first use and reused
...
static struct uk_venus_ring g_reply_ring;
static int                  g_reply_ring_ready;
...
{
	struct uk_virtio_gpu_blob *reply = &g_reply_ring.reply_blob;
	struct uk_venus_encoder enc;
	uint8_t cmd_buf[320];
	uk_gpu_fence_id fence = 0;
	int rc;
	...
	if (!g_reply_ring_ready || g_reply_ring.ctx.id != ctx->id) {
		if (g_reply_ring_ready)
			uk_venus_ring_destroy(dev, &g_reply_ring);
		memset(&g_reply_ring, 0, sizeof(g_reply_ring));
		g_reply_ring.ctx = *ctx;
		g_reply_ring.borrowed_ctx = 1;
		g_reply_ring.ready = 1;
		rc = uk_venus_ring_reply_stream_set(dev, &g_reply_ring);
		if (rc) {
			memset(&g_reply_ring, 0, sizeof(g_reply_ring));
			g_reply_ring_ready = 0;
			return rc;
		}
		g_reply_ring_ready = 1;
	}
	reply = &g_reply_ring.reply_blob;
			       ...
			       char *name_out, unsigned int name_cap)
{
	...
	if (q.overflow)
		return -ENOSPC;
	n = uk_venus_query_reply(dev, ctx, q.buf, q.pos, reply, sizeof(reply));
					      ...
					      uint32_t props_size)
{
	...
	if (!props_out || props_size < sizeof(VkPhysicalDeviceProperties))
		return -EINVAL;
	if (uk_venus_encoder_init(&q, qbuf, sizeof(qbuf)))
		return -EINVAL;
	uk_venus_encode_vkGetPhysicalDeviceProperties(&q, physdev_handle);
	...
	memset(props_out, 0, props_size);
	dec.cur = reply;
	dec.end = reply + (size_t)n;
	dec.fatal = 0;
	vn_decode_vkGetPhysicalDeviceProperties_reply(&dec,
		(VkPhysicalDevice)(uintptr_t)physdev_handle,
		(VkPhysicalDeviceProperties *)props_out);
	return dec.fatal ? -1 : 0;
}
				       ...
				       uint32_t *type_bits_out)
{
	...
	if (q.overflow)
		return -ENOSPC;
	n = uk_venus_query_reply(dev, ctx, q.buf, q.pos, r, sizeof(r));
	if (n < 32)
		return -1;
	memcpy(&cmd_type, r, 4);
