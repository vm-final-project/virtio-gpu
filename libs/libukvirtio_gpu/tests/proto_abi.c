 ...
 * public uk/ API. Until libs/libukvirtio_gpu/protocol/virtio_gpu_proto.h exists,
...
#define FAIL_IF(cond, code) \
	do { if (cond) { printf("proto_abi FAIL code=%d line=%d\n", (code), __LINE__); return (code); } } while (0)
...
int main(void)
{
	...
	printf("proto_abi passed ctrl_hdr=%zu display_info=%zu edid=%zu\n",
