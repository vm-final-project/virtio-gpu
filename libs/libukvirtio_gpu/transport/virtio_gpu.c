/*
 * Mesa alignment:
 *   Analogue: src/virtio/vulkan/vn_renderer_virtgpu.c transport helpers.
 *   Same: native device open, feature/capset/context/blob/submit ordering.
 *   VOGUE adaptation: Unikraft VirtIO queues instead of Linux DRM ioctls.
 */
#include <errno.h>
#include <stdint.h>
#include <stddef.h>
...
int uk_virtio_gpu_gl_venus_requirements_get(struct uk_virtio_gpu_dev *d,
		struct uk_virtio_gpu_caps *caps, const char **status)
{
	struct uk_virtio_gpu_caps local;
	int rc;

	if (!d)
		return -EINVAL;
	rc = uk_virtio_gpu_gl_caps_get(d, &local);
	if (rc) {
		if (status)
			*status = "blocked:caps-query-failed";
		return rc;
	}
	if (caps)
		*caps = local;
	if (!local.has_resource_blob) {
		if (status)
			*status = "blocked:resource-blob-missing";
		return -ENOTSUP;
	}
	if (!local.has_context_init) {
		if (status)
			*status = "blocked:context-init-missing";
		return -ENOTSUP;
	}
	if (!local.has_host_visible) {
		if (status)
			*status = "blocked:host-visible-missing";
		return -ENOTSUP;
	}
	if (status)
		*status = "pass";
