...
/*
 * Mesa alignment:
 *   Analogue: Vulkan loader/entrypoint glue, but not Mesa's dynamic ICD loader.
 *   Same: resolve Vulkan-Hpp dispatch through a proc-address table.
 *   VOGUE adaptation: static single-image dispatch storage.
 */
//
// hpp/vk_hpp_loader.cpp — vulkan.hpp DispatchLoaderDynamic storage for vk.ggml-dispatch.
//
...
// libukvulkan_venus → VirtIO-GPU SUBMIT_3D.
//
// Link order requirement:
//   libvulkan.a must come before ggml-vulkan.o in the final link so
...
extern "C" {
// Provided by runtime/vk_entrypoints.c: our static Venus-backed proc-addr fn.
...
// Called from uk_vulkan_init() after the Venus context is open.
// Wires vulkan.hpp's global dispatcher to use our static proc-addr table.
extern "C" void uk_vulkan_dispatcher_init_with_proc_addr(void)
{
    ...
    auto raw_pfn = uk_vulkan_get_instance_proc_addr_fn();
