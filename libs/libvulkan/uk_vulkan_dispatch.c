...
/*
 * libvulkan/uk_vulkan_dispatch.c — Static Vulkan C ABI for ggml-vulkan
 *
...
static struct uk_vulkan_venus_dev    g_venus;
static int                           g_disp_initialized;
/* Convenience: points into g_venus.ctx after the native Venus driver opens */
...
int uk_vulkan_init(void)
{
     ...
     * Venus context. Works on real hardware and against the fake backend. */
    if (uk_vulkan_venus_open(&g_venus, 0) != 0)
        return -1;

    g_gpu = uk_vulkan_venus_gpu(&g_venus);
    g_ctx = uk_vulkan_venus_ctx(&g_venus);
...
void uk_vulkan_get_info(struct uk_vulkan_info *out)
{
    if (!out)
        return;
    out->initialized   = g_disp_initialized;
...
void uk_vulkan_shutdown(void)
{
    if (!g_disp_initialized)
        return;
    if (g_ring_ready) {
        uk_venus_ring_unregister(g_gpu, &g_ring);
        uk_venus_ring_destroy(g_gpu, &g_ring);
        g_ring_ready = 0;
    }
    g_disp_initialized = 0;
}
